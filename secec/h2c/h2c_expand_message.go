...
func expandMessageXMD(out []byte, hFunc crypto.Hash, domainSeparator, message []byte) error {
	...
	if bInBytes < 2*kay/8 {
		return fmt.Errorf("secp256k1/secec/h2c: b_in_bytes insufficiently large: %d", bInBytes)
	}
	if lenInBytes == 0 || lenInBytes > math.MaxUint16 {
		return fmt.Errorf("secp256k1/secec/h2c: len_in_bytes out of range: %d", lenInBytes)
	}
	...
	if ell > 255 {
		return fmt.Errorf("secp256k1/secec/h2c: ell out of range: %d", ell)
	}
